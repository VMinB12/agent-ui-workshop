...
const SuggestionItem = memo(({ suggestion, onSuggestionClick }: SuggestionItemProps) => {
  const handleClick = useCallback(() => onSuggestionClick(suggestion.value), [onSuggestionClick, suggestion.value])
    ...
    <div className="grid gap-4">
      <Suggestions>
        {suggestions.map((suggestion) => (
          <SuggestionItem key={suggestion.key} onSuggestionClick={handleSuggestionClick} suggestion={suggestion} />
        ))}
      </Suggestions>
      <PromptInput onSubmit={handleSubmit}>
        <PromptInputTextarea onChange={handleTextChange} placeholder="Ask me about anything..." value={text} />
        <PromptInputFooter>
          <PromptInputTools>
            <PromptInputButton>
              <PlusIcon size={16} />
