...
const CheckpointItem = memo(({ checkpoint, onRestore }: CheckpointItemProps) => {
  const handleClick = useCallback(() => onRestore(checkpoint.messageCount), [onRestore, checkpoint.messageCount])
  return (
    <Checkpoint>
      <CheckpointIcon />
...
const Example = () => {
  ...
  const [checkpoints] = useState([{ messageCount: 2, timestamp: new Date(Date.now() - 3_600_000) }])
