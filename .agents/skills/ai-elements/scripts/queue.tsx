...
const TodoItem = memo(({ todo, onRemove }: TodoItemProps) => {
  ...
  return (
    <QueueItem key={todo.id}>
      ...
      {todo.description && <QueueItemDescription completed={isCompleted}>{todo.description}</QueueItemDescription>}
    </QueueItem>
  )
})
                        ...
                        {message.parts
                          ...
                          .map((file) => {
                            if (file.mediaType?.startsWith('image/') && file.url) {
                              return (
                                <QueueItemImage alt={file.filename || 'attachment'} key={file.url} src={file.url} />
                              )
                            }
                            return <QueueItemFile key={file.url}>{file.filename || 'file'}</QueueItemFile>
                          })}
                      </QueueItemAttachment>
                    )}
                  </QueueItem>
