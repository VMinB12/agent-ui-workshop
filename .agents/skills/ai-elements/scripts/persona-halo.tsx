  ...
  return (
    <Tooltip key={state.state}>
      <TooltipTrigger asChild>
        <Button onClick={handleClick} size="icon-sm" variant={currentState === state.state ? 'default' : 'outline'}>
  ...
  return (
    <div className="flex size-full flex-col items-center justify-center gap-4">
      ...
      <ButtonGroup orientation="horizontal">
        {states.map((state) => (
          <StateButton currentState={currentState} key={state.state} onStateChange={handleStateChange} state={state} />
        ))}
      </ButtonGroup>
    </div>
  )
